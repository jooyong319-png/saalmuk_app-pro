...
import { useInterest } from "../components/community/InterestContext";
import { useRecent } from "../components/community/RecentContext";
import InterestGroupSheet from "../components/community/InterestGroupSheet";
import type { PostData } from "../components/community/types";
import { initialPosts } from "../components/community/postsData";
import {
  getGalleryById,
  GalleryData,
} from "../components/community/galleryData";
...
const cardAd = {
  ...
  media: {
    type: "image" as const,
    src: "https://picsum.photos/800/450?random=ad5",
  },
  title: "메이플스토리M 신규 서버 오픈! 사전예약하고 레전드 장비 받자",
  link: "https://example.com",
};

const listAd = {
  id: "ad-channel-list",
  thumbnail: "https://picsum.photos/100/100?random=ad6",
  name: "메이플M 사전예약",
  description: "지금 사전예약하면 레전드 무기 + 100레벨 달성 지원 패키지 증정!",
  link: "https://example.com",
};
...
interface ChannelDetailProps {
  ...
  galleryId?: string; // 갤러리 ID (없으면 기본값 사용)
}
...
function ChannelDetailContent({
  ...
  const { isInInterest, getChannelGroup, toggleInterest } = useInterest();
  const { addRecent } = useRecent();

  // ===== 갤러리 정보 조회 =====
  const galleryInfo =
    (galleryId ? getGalleryById(galleryId) : null) || defaultGallery;

  // ===== 갤러리 방문 기록 추가 =====
  useEffect(() => {
    if (galleryInfo && galleryInfo.id) {
      addRecent({
        id: galleryInfo.id,
        name: galleryInfo.name,
        icon: galleryInfo.icon || "📁",
  ...
  // ===== 갤러리별 카테고리 탭 =====
  const categoryTabs = galleryInfo.categoryTabs || defaultCategoryTabs;

  // ===== 정렬 UI 타입 (sort: 인증글/베스트/전체글, toggle: 최신순/인기순) =====
  const useSortToggle = galleryInfo.sortType === "toggle";
  ...
  const isGalleryInInterest = isInInterest(galleryInfo.id);
  const currentGroup = getChannelGroup(galleryInfo.id);
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (galleryInfoRef.current) {
        const rect = galleryInfoRef.current.getBoundingClientRect();
        // 갤러리 정보 섹션의 하단이 헤더(57px) 아래로 들어가면 타이틀 표시
        setShowHeaderTitle(rect.bottom <= 57);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => {
              ...
              const added = toggleInterest(channel);
              if (added) {
                showToast(TOAST_MESSAGES.INTEREST_ADDED("기본"), {
                  type: "interest",
                  actionLabel: "변경",
                  onAction: () => setShowInterestSheet(true),
                });
              } else {
                showToast(TOAST_MESSAGES.INTEREST_REMOVED(galleryInfo.name), {
                  type: "success",
                });
              }
            }}
          ...
          >
            <svg
              ...
              fill={isGalleryInInterest ? "currentColor" : "none"}
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            >
              <span>
                매니저 {galleryInfo.manager || "없음"} | 부매니저{" "}
                {galleryInfo.subManagers?.join(", ") || "없음"}
              </span>
              <svg
      ...
      {/* ===== 카테고리 탭 (가로 스크롤) ===== */}
      <div className="sticky top-[57px] z-40 bg-white px-4 pt-2 pb-1 overflow-x-auto scrollbar-hide">
      ...
      {/* ===== 정렬 옵션 + 뷰 전환 ===== */}
      <div className="sticky top-[85px] z-40 bg-white flex items-center gap-1 px-4 py-2 border-b border-gray-100">
        {useSortToggle ? (
          // 최신순/인기순 토글 버튼 (클릭 시 텍스트 변경)
          <button
            onClick={() =>
              setSortType(sortType === "popular" ? "latest" : "popular")
            }
            className="flex items-center gap-1 text-[13px] font-medium text-gray-600"
          >
      ...
      {showManagementPopup && (
        <GalleryManagementPopup
          ...
          manager={galleryInfo.manager || "없음"}
      ...
      {showInterestSheet && (
        <InterestGroupSheet
          ...
          onClose={() => setShowInterestSheet(false)}
        />
      )}
    </div>
  );
