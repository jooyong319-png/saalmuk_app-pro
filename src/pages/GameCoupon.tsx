...
export default function GameCoupon({
  ...
  const [registerTab, setRegisterTab] = useState<"등록" | "신규">("등록");
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20">
      ...
      {/* 게임이 없나요? */}
      <div className="bg-white px-4 py-6 border-t border-gray-100 pb-24">
        <button
          onClick={() => { setRegisterTab("신규"); setShowRegisterModal(true); }}
          className="w-full flex items-center gap-3 p-4 rounded-2xl border-2 border-dashed border-gray-200 hover:border-[#72C2FF] hover:bg-[#E8F4FD]/30 transition-all"
        >
          <div className="w-10 h-10 rounded-xl border-2 border-dashed border-gray-300 flex items-center justify-center flex-shrink-0">
            <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
          </div>
          <div className="text-left">
            <p className="text-sm font-bold" style={{ color: "#72C2FF" }}>게임이 없나요?</p>
            <p className="text-[11px] text-gray-400 mt-0.5">새 게임쿠폰 등록 신청하기</p>
          </div>
        </button>
      </div>
      ...
      {!selectedCoupon && (
        <button
          onClick={() => { setRegisterTab("등록"); setShowRegisterModal(true); }}
          className="fixed bottom-6 right-4 max-w-md z-40 flex items-center gap-2 pl-4 pr-5 py-3 rounded-full text-white font-bold text-sm shadow-lg hover:shadow-xl active:scale-95 transition-all"
          style={{ background: "linear-gradient(135deg, #72C2FF 0%, #9B6FF5 100%)", boxShadow: "0 4px 20px rgba(114,194,255,0.4)" }}
        >
      ...
      {showRegisterModal && (() => {
        const RegisterModal = () => {
          const [tab, setTab] = useState<"등록" | "신규">(registerTab);
          ...
          const filteredGames = coupons.filter((c) =>
          ...
          const handleSubmit = () => {
            if (tab === "등록") {
              if (!selectedGame || !code.trim() || !reward.trim()) {
                showToastMessage("게임, 쿠폰 코드, 보상 내용을 입력해주세요.");
                return;
              }
              showToastMessage("쿠폰이 등록되었습니다! +100P 지급");
            } else {
              if (!newGameName.trim() || !newCode.trim() || !newReward.trim()) {
                showToastMessage("게임 이름, 쿠폰 코드, 보상 내용을 입력해주세요.");
                return;
              }
              showToastMessage("신규 게임쿠폰 신청이 완료되었습니다! +100P 지급");
            }
          ...
          return (
            <div className="fixed inset-0 z-[9999] flex items-end justify-center">
              ...
              <div
                className="relative bg-white rounded-t-[20px] w-full max-w-md flex flex-col overflow-hidden"
                style={{ height: "90vh", animation: "slideUp 0.3s ease-out" }}
                  ...
                  <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
                    {(["등록", "신규"] as const).map((t) => (
                      <button key={t} onClick={() => setTab(t)}
                        className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${tab === t ? "bg-white text-gray-900 shadow-sm" : "text-gray-400"}`}>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
                  {tab === "등록" ? (
                    <>
                      {/* 게임 선택 */}
                      <div className="relative">
                        <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1">게임 선택 <span className="text-red-500">*</span></label>
                        <button type="button" onClick={() => setShowPicker(!showPicker)}
                          className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border text-sm transition-colors ${showPicker ? "border-[#72C2FF]" : "border-gray-200"} ${selectedGame ? "text-gray-800" : "text-gray-400"}`}>
                          {selectedGame ? (
                            <div className="flex items-center gap-2">
                        ...
                        {showPicker && (
                          <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 overflow-hidden">
                            <div className="flex items-center gap-2 px-3 py-2.5 border-b border-gray-100">
                              ...
                              <input autoFocus value={gameSearch} onChange={(e) => setGameSearch(e.target.value)} placeholder="게임명 검색" className="flex-1 text-sm outline-none placeholder-gray-400" />
                            </div>
                            <div className="max-h-[200px] overflow-y-auto">
                              {filteredGames.map((c) => (
                                <button key={c.id} type="button" onClick={() => { setSelectedGame(c); setShowPicker(false); setGameSearch(""); }}
                              ...
                              {filteredGames.length === 0 && <p className="text-center text-sm text-gray-400 py-6">검색 결과가 없습니다</p>}
                            </div>
                            <button type="button" onClick={() => { setShowPicker(false); setTab("신규"); }}
                      ...
                      <div>
                        <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1">쿠폰 코드 <span className="text-red-500">*</span></label>
                        <input type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="예 : GENSHIN2026"
                      ...
                      <div>
                        <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1">보상 내용 <span className="text-red-500">*</span></label>
                        <input type="text" value={reward} onChange={(e) => setReward(e.target.value)} placeholder="예: 원석 100개 + 모라 50,000"
                          className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm placeholder-gray-300 focus:outline-none focus:border-[#72C2FF]" />
                      </div>
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <input type="text" value={source} onChange={(e) => setSource(e.target.value)} placeholder="예: 공식 트위터"
                    ...
                    <>
                      <div>
                        <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1">게임 이름 <span className="text-red-500">*</span></label>
                        <input type="text" value={newGameName} onChange={(e) => setNewGameName(e.target.value)} placeholder="예: 원신, 블루 아카이브"
                          className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm placeholder-gray-300 focus:outline-none focus:border-[#72C2FF]" />
                      </div>
                      <div>
                        <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1">쿠폰 코드 <span className="text-red-500">*</span></label>
                        <input type="text" value={newCode} onChange={(e) => setNewCode(e.target.value.toUpperCase())} placeholder="예 : GENSHIN2026"
                          className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm placeholder-gray-300 focus:outline-none focus:border-[#72C2FF]" />
                      </div>
                      <div>
                        <label className="text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1">보상 내용 <span className="text-red-500">*</span></label>
                        <input type="text" value={newReward} onChange={(e) => setNewReward(e.target.value)} placeholder="예: 원석 100개 + 모라 50,000"
                          className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm placeholder-gray-300 focus:outline-none focus:border-[#72C2FF]" />
                      </div>
                      <div className="flex gap-3">
                        <div className="flex-1">
                          <label className="text-sm font-semibold text-gray-700 mb-1.5 block">만료일 <span className="text-gray-400 font-normal">(선택)</span></label>
                          <input type="date" value={newExpireDate} onChange={(e) => setNewExpireDate(e.target.value)}
                            className="w-full border border-gray-200 rounded-xl px-3 py-3 text-sm text-gray-500 focus:outline-none focus:border-[#72C2FF]" />
                        </div>
                        <div className="flex-1">
                          <label className="text-sm font-semibold text-gray-700 mb-1.5 block">출처 <span className="text-gray-400 font-normal">(선택)</span></label>
                          <input type="text" value={newSource} onChange={(e) => setNewSource(e.target.value)} placeholder="예: 공식 트위터"
                            className="w-full border border-gray-200 rounded-xl px-3 py-3 text-sm placeholder-gray-300 focus:outline-none focus:border-[#72C2FF]" />
                        </div>
                      </div>
